...
public class ApiChatController {
	...
	public JSONObject searchChat(String area) throws Exception {
		Server server=serverListService.findServerByKey(area);
		if (server==null) {
			return null;
		}
		List<ChatEntity> list=playerService.findAllChat(server);
		
		JSONArray chatArray=JSONArray.fromObject(list);
		JSONObject jsonObject=new JSONObject();
		
		jsonObject.accumulate("chatList", chatArray);
	    return jsonObject;
	}
	@RequestMapping("/api/chat/kickPlayer")
	...
	@RequestMapping("/api/chat/canclePlayer")
	public String canclePlayer(String ipid,String area) throws IOException, URISyntaxException {
		String body = "{\"module\":\"report\",\"cmd\":\"HandlePerson\",\"args\":{\"gm\":1,\"id\":\""+ipid+"\",\"serverkey\":\""+area+"\",\"type\":\"cancelbanact\",\"key\":\""+ipid+"\"}}";
		int status=HttpUtils.POST(ip, "/backend/", body);
		if (status==200) {
			return "成功解禁";
		}else {
